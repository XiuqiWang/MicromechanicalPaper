...
def store_sal_id_data(data, ID_Particle, coe_h, dt, N_inter, D):
    ...
    for i in range(len(ID_Particle)):
        ...
        Moms_coli, IDmobile_vec, IDvzri_vec, ezi, exzi, Vi_i, Vr_i, dxim_i, theta_i = findSaltationID(e, Vx, Vz, z, thre_e, dt)
        ...
        if IDvzri_vec.size > 0:
            ...
            for j in range(IDvzri_vec.shape[0]):
                ...
                IM_vector_t[idx - 1].append([Vi_i[j], IDim_i[j], IDre_i[j], xim_i[j], xre_i[j], x_col[j], i, theta_i[j]])
...
def findSaltationID(e, Vxi, Vzi, Zi, thre_e, dt):
    ...
    return Moments_col, np.array(IDmobile), np.array(IDvzri_vec), np.array(ez_vector), np.array(exz_vector), np.array(Vi_vec), np.array(Vr_vec), np.array(dxim_vec), np.array(Thetai_vec)
...
def Findez(ID_next, Vzi, Vxzi, Zi, thre_e):
    ...
    if IDvzri.size > 0:
        ...
        # IDvzri = IDvzri[Zi[IDvzri[:, 0]] <= 50*0.00025]
