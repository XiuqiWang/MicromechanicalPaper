...
def BinUincUsal(Uim, Thetaim, Usal, velsal_bins):
    Uim = np.array(Uim)
    Thetaim = np.array(Thetaim)
    Usals = np.array(Usal)
    Uimx = Uim*np.cos(np.deg2rad(Thetaim))
    Uimz = Uim*np.sin(np.deg2rad(Thetaim))
    ...
    for i in range(len(velsal_bins)-1):
        ...
        if np.any(idx):  # Check if there are elements in this range
            Uimx_in_bin = Uimx[idx]
            Uimz_in_bin = Uimz[idx]
    ...
    return np.array(Uim_mean), np.array(Uim_stderr), np.array(Usal_median)
