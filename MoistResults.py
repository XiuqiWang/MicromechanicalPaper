...
U_fit_new = np.linspace(min(Uthetaplot/constant), max(Uthetaplot/constant), 100)
...
plt.figure(figsize=(6,5))
for i in range(5):
    plt.scatter(UthetaDplot/constant, mean_thetaD[i], s=np.sqrt(N_UD[i])*5, label=rf'$\Omega$={Omega[i]}%', color=colors[i])
...
UD_tbfit = np.tile(UthetaDplot, 5)
N_UD_tbfit = list(chain.from_iterable(N_UD.values()))
stderr_thetaD_tbfit = list(chain.from_iterable(stderr_thetaD.values()))
...
stderr_thetaD_tbfit_new = np.deg2rad(np.array(stderr_thetaD_tbfit))[valid_indices]
valid_mask = np.where(~np.isnan(stderr_thetaD_tbfit_new))[0]
UD_tbfit_new = UD_tbfit_new[valid_mask]
thetaD_tbfit_new = thetaD_tbfit_new[valid_mask]
stderr_thetaD_tbfit_new = stderr_thetaD_tbfit_new[valid_mask]
params_thetaD, _ = curve_fit(fit_arcsin_thetainf, UD_tbfit_new, thetaD_tbfit_new, p0=[0.4, 20, 5], 
                              sigma=stderr_thetaD_tbfit_new, absolute_sigma=True, maxfev=10000)
thetainf, A_thetaD, B_thetaD = params_thetaD
print(f'thetainf={thetainf:2f}, A_thetaD={A_thetaD:.4f} B_thetaD={B_thetaD:.4f}')

UD_fit = np.linspace(min(UD_tbfit/constant), max(UD_tbfit/constant), 100) #for the fit
...
# Now compute R²
r2_thetaD = weighted_r2(thetaD_tbfit_new, thetaD_fit_resampled, 1/stderr_thetaD_tbfit_new**2)
...
plt.subplot(1,2,2)
for i in range(5):
    plt.errorbar(UthetaDplot/constant, mean_thetaD[i], yerr=stderr_thetaD[i], fmt='o', capsize=5, color=colors[i])
...
plt.ylabel(r'$\theta_{re}$ [$^\circ$]', fontsize=14)
plt.xlim(left=0)
plt.legend(fontsize=12)
plt.tight_layout()
plt.show()   
